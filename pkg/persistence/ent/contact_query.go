...
func (_q *ContactQuery) QueryOwner() *PartyQuery {
	query := (&PartyClient{config: _q.config}).Query()
...
func (_q *ContactQuery) WithOwner(opts ...func(*PartyQuery)) *ContactQuery {
	query := (&PartyClient{config: _q.config}).Query()
...
func (_q *ContactQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Contact, error) {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *Contact, e *Party) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ContactQuery) loadOwner(ctx context.Context, query *PartyQuery, nodes []*Contact, init func(*Contact), assign func(*Contact, *Party)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Contact)
	for i := range nodes {
		if nodes[i].party_contacts == nil {
			continue
		}
		fk := *nodes[i].party_contacts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "party_contacts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
